            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
               { postData && postData.map((post,index) => (
                <article>
                    ...
                    <span className="block h-64 relative rounded shadow leading-snug bg-white border-l-8 border-bg-green-400" key={index}>
                        ...
                        <span className="block relative h-full flex justify-end items-end pr-4 pb-4">
                            <h3 className="text-gray-800 text-lg font-blog px-3 py-4 bg-green-400 text-red-100 bg-opacity-75 rounded">{post.title}</h3>
                        </span>
                    </span>
                    </Link>
                </article>
